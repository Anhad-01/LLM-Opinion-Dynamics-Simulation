    ...
    if not os.path.exists("runs"):
        ...
        if not all_runs:
            ...
            if selected_run:
                        ...
                        for line in f:
                            if "Topic: " in line:
                                ...
                                if ", Controller:" in topic_part:
                                    ...
                                    controller_str = topic_part.split(", Controller:")[1].strip()
                                elif ", Topology:" in topic_part:
                                    topic_str = topic_part.split(", Topology:")[0].strip()
                                    topology_str = topic_part.split(", Topology:")[1].strip()
                                else:
                ...
                if topology_str == "Filter Bubble":
                    total_agents = df['Agent_ID'].nunique()
                    mid = total_agents // 2
                    group_a = [f"Agent_{i}" for i in range(2, mid + 2)]
                    group_b = [f"Agent_{i}" for i in range(mid + 2, total_agents + 1)]
                    st.info(f"**Filter Bubble Active**\n\n**Bridge Agent:** Agent_1\n\n**Group A:** {', '.join(group_a)}\n\n**Group B:** {', '.join(group_b)}")
                
                elif topology_str == "Ring":
                    st.info("**Ring Topology Active**\n\nUnlike Filter Bubbles which have distinct groups, the Ring topology forces every agent into a continuous circular chain. An agent will **only** read the debate history generated by their immediate left and right neighbors (e.g., Agent_3 only sees Agent_2 and Agent_4). Opinions propagate linearly over multiple rounds.")
                    
                elif controller_str:
                    total_agents = df['Agent_ID'].nunique()
                    st.info(f"**Controller Agent Intervention Active**\n\n**Injected Controller Persona:** {controller_str}\n\n**Controller Agent Identity:** Agent_{total_agents}")
