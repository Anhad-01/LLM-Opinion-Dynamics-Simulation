...
if not API_KEYS:
    ...
    if scenario_choice == "Standard Global Townhall":
        df = asyncio.run(run_standard_simulation(live_feed, NUM_AGENTS, NUM_ROUNDS, TOPIC))
    elif scenario_choice == "Controller Agent Intervention":
        df = asyncio.run(run_controller_simulation(live_feed, NUM_AGENTS, NUM_ROUNDS, TOPIC, controller_type))
    elif scenario_choice == "Restricted Network Topology":
        df = asyncio.run(run_topology_simulation(live_feed, NUM_AGENTS, NUM_ROUNDS, TOPIC, topology_type))
